...
Generated by `g init`. All values have defaults and the file is not required.
...
| Key | Type | Default | Description |
|---|---|---|---|
| `agent` | string | `"oak"` | Default active agent when `GROVE_AGENT` not set |
...
1. `GROVE_AGENT` env var or `defaults.agent` in config — selects active agent
