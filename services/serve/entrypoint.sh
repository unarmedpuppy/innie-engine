...
AGENT=${GROVE_AGENT:-${INNIE_AGENT:-grove}}
HOME_DIR=${GROVE_HOME:-${INNIE_HOME:-/root/.grove}}
HOST=${GROVE_SERVE_BIND:-${INNIE_SERVE_BIND:-0.0.0.0}}
PORT=${GROVE_SERVE_PORT:-${INNIE_SERVE_PORT:-8013}}
AGENT_DIR="${HOME_DIR}/agents/${AGENT}"

echo "[grove-serve] Starting API server. Agent=${AGENT} Home=${HOME_DIR} Bind=${HOST}:${PORT}"
...
if [ -n "${GITEA_TOKEN:-}" ]; then
    ...
    gosu appuser git config --global user.email "${GIT_AUTHOR_EMAIL:-${AGENT}@grove.local}"
    gosu appuser git config --global user.name "${GIT_AUTHOR_NAME:-${AGENT}}"
    echo "[grove-serve] Git credentials configured for ${GITEA_HOST}"
fi
...
if [ -d "/app/bootstrap/${AGENT}" ]; then
    ...
    echo "[grove-serve] Bootstrap sync complete for ${AGENT}"
fi
...
setup_ssh_keys() {
    ...
    if [ ! -f "$ssh_dir/id_ed25519" ]; then
        echo "[grove-serve] Generating SSH key for appuser..."
        gosu appuser ssh-keygen -t ed25519 -f "$ssh_dir/id_ed25519" -N "" -C "${AGENT}@grove"
        echo "[grove-serve] SSH public key (add as deploy key on agent-memory repo):"
...
setup_memory_remote() {
    ...
    if [ ! -d "${data_dir}/.git" ]; then
        echo "[grove-serve] Initializing ${data_dir} as git repo..."
        gosu appuser git -C "$data_dir" init
        gosu appuser git -C "$data_dir" remote add origin "$remote_url"
        echo "[grove-serve] Memory remote added: $remote_url"
    else
        local current
        ...
        if [ "$current" != "$remote_url" ]; then
            gosu appuser git -C "$data_dir" remote set-url origin "$remote_url" 2>/dev/null || \
                gosu appuser git -C "$data_dir" remote add origin "$remote_url"
            echo "[grove-serve] Memory remote updated: $remote_url"
        else
            echo "[grove-serve] Memory remote already set"
        fi
    fi
}
...
setup_workspace() {
    ...
    if [ -z "${GITEA_TOKEN:-}" ]; then
        echo "[grove-serve] GITEA_TOKEN not set, skipping workspace bootstrap"
        return
    fi

    gosu appuser mkdir -p "$workspace"
    echo "[grove-serve] Bootstrapping homelab workspace..."
    ...
    repos=$(curl -sf "http://gitea:3000/api/v1/orgs/homelab/repos?limit=50" \
        ...
        echo "[grove-serve] Warning: failed to fetch homelab repos, skipping"
        return
    }
    ...
    while IFS= read -r repo; do
        ...
        if [ ! -d "$repo_dir/.git" ]; then
            gosu appuser git clone --quiet "$clone_url" "$repo_dir" 2>/dev/null \
                && cloned=$((cloned+1)) \
                || echo "[grove-serve] Warning: failed to clone $repo"
        else
    ...
    echo "[grove-serve] Workspace ready: ${cloned} cloned, ${pulled} updated"
}
...
exec gosu appuser g serve --host "$HOST" --port "$PORT"
