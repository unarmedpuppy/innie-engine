...
def status():
    ...
    # Trace stats
    from grove.core.trace import trace_db_path

    tdb = trace_db_path(agent)
    if tdb.exists():
        try:
            from grove.core.trace import get_stats, open_trace_db
...
def doctor():
    ...
    provider = get("embedding.provider", "none")
    if provider != "none":
        from grove.core.search import get_embedding_dims
...
def decay(
    ...
    from grove.core.decay import decay_all
