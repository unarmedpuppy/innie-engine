...
def run(
    ...
    if is_interactive():
        from grove.tui.apps.heartbeat import HeartbeatApp
    ...
    from grove.core.collector import collect_all
    ...
    extraction = None
    try:
        from grove.heartbeat.extract import extract
    ...
    if extraction is not None:
        ...
        from grove.heartbeat.route import route_all
...
def _self_update():
    ...
    dist_info = next(
        Path(sys.executable).parent.parent.glob(
            "lib/python*/site-packages/grove*.dist-info"
        ),
        None,
    )
    ...
    if result.returncode == 0:
        console.print("  [green]✓[/green] grove updated")
...
def hb_status():
    ...
    from grove.core.config import get as cfg_get
    ...
    if resolved_provider == "openclaw":
        try:
            from grove.heartbeat.extract import _resolve_openclaw
