...
async def lifespan(app: FastAPI):
    ...
    from grove.core.agent_env import inject_into_os_env
...
def _detect_service_info(agent: str) -> dict:
    ...
    if install_url:
        if install_url.startswith("file://"):
            ...
            install_cmd = f"uv tool install 'grove[serve] @ {install_url}'"
...
async def health():
    ...
    from grove.channels.loader import get_channel_health
...
async def _run_wake(agent: str) -> None:
    ...
    from grove.serve.scheduler import _load_schedule, trigger_job
    ...
    g_bin = _Path(sys.executable).parent / "g"
...
async def agent_skills():
    from grove.skills.registry import discover_skills
    agent = paths.active_agent()
...
async def agent_schedule():
    from grove.serve.scheduler import _load_schedule, _scheduler
    agent = paths.active_agent()
...
async def trigger_schedule_job(job_name: str, background_tasks: BackgroundTasks):
    ...
    from grove.serve.scheduler import trigger_job
    agent = paths.active_agent()
...
async def ingest_trace_event(request: Request):
    ...
    from grove.core.trace import end_session, open_trace_db, record_span, start_session
...
async def get_trace_api(session_id: str):
    ...
    from grove.core.trace import get_session, open_trace_db, trace_db_path
