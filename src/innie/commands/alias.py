...
def generate_all():
    ...
    Usage in ~/.zshrc:
        source <(innie alias generate-all)

    Each agent gets two aliases:
        <agent>         → innie launch <agent>              (default Anthropic oauth)
        <agent>-claude  → innie launch <agent> --mode claude (local proxy routing)
    """
    from innie.core.profile import list_agents

    # Only generate aliases for agents that have a SOUL.md (real agents, not stubs)
    agents = [a for a in list_agents() if (paths.agent_dir(a) / "SOUL.md").exists()]
    lines = ["# innie agent aliases — generated by `innie alias generate-all`"]
    for name in agents:
        lines.append(f"alias {name}='innie launch {name}'")
        lines.append(f"alias {name}-claude='innie launch {name} --mode claude'")
