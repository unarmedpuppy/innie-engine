...
def _step_version_check(agent: str, force_update: bool) -> str:
    ...
    from innie import __version__ as installed_ver
    ...
    if expanded:
        ...
        if not needs_update:
            console.print("  [dim]Remote source — run `innie update` for full upgrade check.[/dim]")
...
def _step_run_heartbeat(agent: str, skip: bool) -> None:
    ...
    innie_bin = Path(sys.executable).parent / "innie"
